...
export default function SubCategoryModal() {
  ...
  const { modal, closeModal, categories, addSubCategory, updateSubCategory } =
    useCategoriesStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && subCategory) {
        const updated = await DepartmentsService.updateSubDepartment(
          subCategory.id,
          {
            name,
            parentId,
        ...
        addToast({
          message:
            locale?.categories?.toasts?.subCategoryUpdated ||
            "Sub category updated",
          type: "success",
        });
      } else {
        const created = await DepartmentsService.createSubDepartment({
          name,
          parentId,
        });
        addSubCategory({
        ...
        addToast({
          message:
            locale?.categories?.toasts?.subCategoryCreated ||
  ...
  return (
    <Modal
      title={
        isEdit
          ? locale.categories.subCategoryModal.editTitle
          : locale.categories.subCategoryModal.addTitle
      }
      onClose={closeModal}
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">
            {locale.categories.subCategoryModal.nameLabel}
          </label>
          <input
            type="text"
            value={name}
            ...
            placeholder={locale.categories.subCategoryModal.namePlaceholder}
            required
          />
          {errors.name && (
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">
            {locale.categories.subCategoryModal.parentLabel}
          </label>
          <select
            value={parentId}
          ...
          >
            <option value="" disabled>
              {locale.categories.subCategoryModal.parentPlaceholder}
            </option>
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          >
            {locale.categories.subCategoryModal.cancel}
          </button>
          <button
            type="submit"
            disabled={isLoading}
          ...
          >
            {isLoading
              ? locale.categories.subCategoryModal.saving
              : isEdit
              ? locale.categories.subCategoryModal.saveChanges
              : locale.categories.subCategoryModal.create}
          </button>
        </div>
      </form>
    </Modal>
