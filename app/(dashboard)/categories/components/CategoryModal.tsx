...
export default function CategoryModal() {
    ...
    return (
        <Modal title={isEdit ? locale.categories.categoryModal.editTitle : locale.categories.categoryModal.addTitle} onClose={closeModal}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">
                        {locale.categories.categoryModal.nameLabel}
                    </label>
                    <input
                        type="text"
                        value={name}
                        ...
                        placeholder={locale.categories.categoryModal.namePlaceholder}
                        required
                    />
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">
                        {locale.categories.categoryModal.visibilityLabel}
                    </label>
                    <select
                        value={visibility}
                    ...
                    >
                        <option value="PUBLIC">{locale.categories.categoryModal.visibilityPublic}</option>
                ...
                {!isEdit && (
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">
                            {locale.categories.categoryModal.initialKnowledgeLabel}
                        </label>
                        <textarea
                            value={knowledge}
                            ...
                            placeholder={locale.categories.categoryModal.initialKnowledgePlaceholder}
                        />
                    </div>
                )}
                ...
                <div className="flex justify-end gap-3 pt-2">
                    ...
                    >
                        {locale.categories.categoryModal.cancel}
                    </button>
                    <button
                        type="submit"
                        disabled={isLoading}
                    ...
                    >
                        {isLoading ? locale.categories.categoryModal.saving : isEdit ? locale.categories.categoryModal.saveChanges : locale.categories.categoryModal.create}
                    </button>
                </div>
            </form>
        </Modal>
