...
export default function CategoryCard({ category, knowledgeChunks }: CategoryCardProps) {
...
function VisibilityBadge({ visibility }: { visibility: string }) {
    ...
    return (
        <span
            className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${isPublic
                    ? "text-green-700 bg-green-100"
                    : "text-slate-700 bg-slate-200"
                }`}
        >
            {isPublic ? <EyeIcon className="w-3 h-3" /> : <EyeSlashIcon className="w-3 h-3" />}
