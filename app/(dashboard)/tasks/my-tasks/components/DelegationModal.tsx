...
export default function DelegationModal({
  ...
  const handleSelect = async (
    ...
    try {
      ...
      addToast({
        message: `Task successfully assigned to ${type === "employee" ? "employee" : "sub-department"}`,
        type: "success",
      });
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      >
        <div className="p-6">
          ...
          <AnimatePresence>
            {showResults && (employees.length > 0 || subDepartments.length > 0) && (
              <motion.div
                    ...
                    {employees.map((employee) => (
                      ...
                      >
                        {employee.user.profilePicture &&
                          ...
                          <img
                            src={`${env("NEXT_PUBLIC_API_URL")}/profile/pictures/${employee.user.profilePicture}`}
                            alt={employee.user.name}
                            className="w-10 h-10 rounded-full object-cover border-2 border-gray-200"
                            onError={() => {
                              setFailedImages((prev) => new Set(prev).add(employee.id));
                            }}
                          />
                        ) : (
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center text-slate-600 font-medium text-sm border-2 border-gray-200">
                            {getInitials(employee.user.name, employee.user.username)}
                          </div>
                        )}
                        <div className="flex-1 text-left">
                          ...
                          <div className="text-sm text-gray-500">
                            ...
                            {employee.user.jobTitle && ` • ${employee.user.jobTitle}`}
                          </div>
                        </div>
                      </motion.button>
                    ))}
