...
import type { AttachmentMetadata } from "@/lib/api/v2/services/filehub-attachment-groups";
...
/**
 * Options for the useFileHubAttachmentGroup hook
 */
interface UseFileHubAttachmentGroupOptions {
...
/**
 * Return type for the useFileHubAttachmentGroup hook
 */
interface UseFileHubAttachmentGroupResult {
...
/**
 * React hook for connecting to and managing FileHub attachment group WebSocket updates
 *
 ...
 */
export function useFileHubAttachmentGroup(
  baseUrl: string,
  groupKey: string,
  options: UseFileHubAttachmentGroupOptions = {}
): UseFileHubAttachmentGroupResult {
  const {
    autoConnect = true,
    reconnectAttempts = 5,
    reconnectDelay = 1000,
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      // Create Socket.IO connection with FileHub namespace
      socketRef.current = io(`${baseUrl}/filehub/attachment-groups`, {
